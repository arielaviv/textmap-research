// OUT-OF-SAMPLE validation (audit follow-up, 2026-07-19): the 93 was measured
// on seeds 2000-2009 — the exact scenes the Tier 0-2 improvements were tuned
// against (in-sample). This run scores the POST-AUDIT config (no hull=, no
// nearest_offstreet scan override, arm-aware tool wording) on 10 FRESH scenes:
// seeds 2020-2029 → lattice residues 20-29, never used during tuning. Same
// families, same hints, new maps — the OptiMind-style instance-generalization
// test. Config otherwise identical to sc-validate.mts.
...
for (let i = 0; i < 10; i++) {
  try {
    const { buildings, streets } = await fetchRealOSM("nyc", 2020 + i, 350);
    scenes.push(
      buildRealScene({
        id: `fresh-${i}`,
        buildings,
...
console.log(`built ${scenes.length} fresh real-NYC scenes (seeds 2020-2029)`);
...
fs.writeFileSync("results-fresh.json", JSON.stringify({ byCat, byQ, composite: (100 * ok) / total, tin, tout }, null, 2));
console.log("wrote results-fresh.json");
